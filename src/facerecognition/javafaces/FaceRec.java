...
public class FaceRec{
	...
	private String numbersList="1234567890";
	Handler fh;
	...
	public MatchResult findMatchResult(String imageFileName,int selectedeigenfaces,double thresholdVal){
		...
		try{
			...
			Matrix2D inputWts = getInputWeights(selectedeigenfaces, inputFace);//Gets the "coordinates"
			System.out.println(inputWts);
			...
			matchingFileName = personList[distanceInfo.getIndex()];
	...
	private double[] getDistances(Matrix2D inputWt) {			
		Matrix2D tempWt = new Matrix2D(avgWeights); //This part puts the weights into a matrix (pretty self explanitory)
	...
	private double[][] calculateWeights(FaceBundle b,int selectedNumOfEigenFaces){
		...
		nameList = new String[b.getImageFileNamesList().size()];
		for(int i=0; i<b.getImageFileNamesList().size();i++){
			nameList[i]=b.getImageFileNamesList().get(i);
			//System.out.println(b.getImageFileNamesList().get(i)); <<was a test line
		}
		nameList=isolateName(nameList);
		personList=uniquePeopleList(nameList);
		
		double[][] arrWts=wts.toArray();
		//System.out.println(arrWts[1][1]); <<Another test line
		avgWeights=getAvgWeights(arrWts);
		
		Matrix2D avgWeightsMat=new Matrix2D(avgWeights);
		return arrWts;
	}
	
	private double[][] getAvgWeights(double[][] weights){ //This FN Averages all of the weights
		double[][]avgs=new double[personList.length][weights[0].length];
		for(int i=0; i<avgs.length; i++){ //loops through rows of to be sorted avg weights
			String currentName=personList[i];
			int faceCount=0;
			for(int j=0; j<weights.length; j++){ //loops through the list of unsorted weights
				if(nameList[j].equals(currentName)){
					for(int k=0; k<avgs[i].length; k++){ //adds each element from a row
						avgs[i][k]+=weights[j][k];
					}
					faceCount++;
				}
			}
			for(int j=0; j<avgs[i].length; j++){
				avgs[i][j]/=(double)(faceCount);
			}
		}
		return avgs;
	}
	private String[] isolateName(String[] names){//This FN takes an array of filepaths and takes out just the names of the people
	...
	private String[]uniquePeopleList(String[] names){//gets a list of separate people, separated by name
		ArrayList<String> uniqueNames=new ArrayList<String>();
		for(int i=0; i<names.length;i++){
			boolean shouldAdd=true;
			for(int j=0; j<uniqueNames.size(); j++){
				if(names[i].equals(uniqueNames.get(j))){
					shouldAdd=false;
					j=uniqueNames.size();
				}
			}
		...
		String[]arrUniqueNames=new String[uniqueNames.size()];
		for (int i=0; i<arrUniqueNames.length; i++){
			arrUniqueNames[i]=uniqueNames.get(i);
		}
		return arrUniqueNames;
	}
		/*for(int i=0; i<names.length-1; i++){
			String currentName=names[i];
			if(currentName.equals("N/A")){
				i++;
			}
			else{
				for(int j=i; j<names.length; j++){
					String checkName=names[j];
					if(checkName.equals("N/A")){
						j++;
					}
					else if (checkName.equals(currentName)){
						names[j]="N/A";
					}	
				}
			}
	...
	public FaceBundle createFaceBundle(List<String> filenames) throws FaceRecError, IOException{
		...
		Matrix2D imagesData = getNormalisedImagesData(bufimgs); ///Important
		...
		return b;	//figure out what the adjusted faces
	}
	...
	private void processCache(String dir,List<String>newFileNames,FaceBundle oldBundle,int selectedNumOfEigenFaces) throws FaceRecError, IOException{
		...
		if(newFileNames.equals(oldFileNames)){
			...
			this.weights = calculateWeights(oldBundle,selectedNumOfEigenFaces); //make this use avg weights for each face
		}
		else{
	...
	public MatchResult processSelections(String faceImageName,String directory,String numofFaces,String threshold) {
		...
		//System.out.println(personList[1]);
		return result;
	}
